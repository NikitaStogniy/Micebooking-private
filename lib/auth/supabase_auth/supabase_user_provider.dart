...
class MicebookingSupabaseUser extends BaseAuthUser {
  MicebookingSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> micebookingSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MicebookingSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
